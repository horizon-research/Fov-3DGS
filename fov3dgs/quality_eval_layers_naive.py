...
hl_base = "4_12_0.01_1.0/all_shared_fr_4_12"
method = "naiveFR"
...
def main():
    for scene in all_scenes:
        ...
        model_dir = os.path.join(model_base, scene) 

        for i in range(len(method_dir_list)):
            method_path = os.path.join(model_dir, method_dir_list[i])
            hl = os.path.join(model_dir, hl_base, "highest_levels.pth")
            of = os.path.join(model_dir, hl_base, str(i) )
            iterations = iterations_list[i]
            render_command = f"python3 render_naive.py -s {scene_dir} -m {method_path} --eval --skip_train --iteration {iterations} --target_layer {i} -hl {hl} --out_folder {of}"
        ...
        for i in range(len(method_dir_list)):
            ...
            iterations = iterations_list[i]
            ps = ps_list[i]
            result_path = os.path.join(of, "test", f"ours_{iterations}")
