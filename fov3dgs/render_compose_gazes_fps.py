...
from gaussian_renderer_fov import render
import torchvision
from utils.general_utils import safe_state
from argparse import ArgumentParser
from arguments import ModelParams, PipelineParams, get_combined_args
from gaussian_renderer_fov import GaussianModel
import sys

def render_set(out_path, name, iteration, views, gaussians, pipeline, background, highest_levels, shs_dcs, opacities):
    ...
    for gi, gaze in enumerate(gaze_samples):
        ...
        for i in range(10):
            blending_rendering = render(first_view, gaussians, background, alpha=0.05, gazeArray = gazeArray, blending=True, starter= starter, ender= ender, highest_levels=highest_levels, shs_dcs=shs_dcs, opacities=opacities)["render"]
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        # load mask info
        mask_path = os.path.join(args.base_folder,f"composed_{args.layer_num}_{args.max_pooling_size}", "highest_levels.pt")
        highest_levels = torch.load(mask_path)
        shs_dcs_path = os.path.join(args.base_folder,f"composed_{args.layer_num}_{args.max_pooling_size}", "shs_dcs.pt")
        shs_dcs = torch.load(shs_dcs_path)
        opacities_path = os.path.join(args.base_folder,f"composed_{args.layer_num}_{args.max_pooling_size}", "opacities.pt")
        opacities = torch.load(opacities_path)

        bg_color = [1,1,1] if dataset.white_background else [0, 0, 0]
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

        out_path = os.path.join(args.base_folder, f"composed_{args.layer_num}_{args.max_pooling_size}")

        # if not skip_train:
        #      render_set(out_path, "train", 0, scene.getTrainCameras(), gaussians, pipeline, background, highest_levels, shs_dcs, opacities)

        if not skip_test:
             render_set(out_path, "test", 0, scene.getTestCameras(), gaussians, pipeline, background, highest_levels, shs_dcs, opacities)
