...
renderCUDA_blending(const int P,
	...
	const bool* __restrict__  tile_skips,
	...
	bool tile_skip = tile_skips[current_tile_idx];
	if (!blending || tile_skip){
		return;
	}
	...
	L1_i = int(estimated_pix_level);
	x = (estimated_pix_level - (float(L1_i) + start_blend) )  / blend_width;

	if (x < 0 && L1_i != cur_level) {
		done = true;
	}
	...
	if (inside)
	{
		...
		if (L1_i == cur_level){
			...
			used_weight = 1.0 - L1_w;
		}

		for (int ch = 0; ch < CHANNELS; ch++)
			out_color[ch * H * W + pix_id] = C1[ch] * used_weight;
	}
