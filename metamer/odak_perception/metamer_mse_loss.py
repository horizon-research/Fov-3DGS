...
class MetamerMSELoss():
    ...
    def __init__(self, device=torch.device("cpu"),
                 ...
                 n_pyramid_levels=5, n_orientations=2, equi=False, loss_type="L1", avg_pooling_downup=False):
        """
        Parameters
        ----------
        alpha                   : float
        ...
        self.metameric_loss = MetamericLoss(device=device, alpha=alpha, real_image_width=real_image_width,
                                            ...
                                            ,use_bilinear_downup=avg_pooling_downup)
        if loss_type == "L1":
    ...
    def gen_metamer(self, image, gaze):
        ...
        target_stats = self.metameric_loss.calc_statsmaps(
            image, gaze=gaze, alpha=self.metameric_loss.alpha, real_image_width=self.metameric_loss.real_image_width, real_viewing_distance=self.metameric_loss.real_viewing_distance, mode=self.metameric_loss.mode)
